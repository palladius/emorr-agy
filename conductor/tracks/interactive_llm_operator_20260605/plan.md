...
- [x] Task 3.1: Implement PIN gate check (`4242`) in the Telegram bot update handler in `main.go` (lock server on startup, terminate on 3 wrong attempts).
- [x] Task 3.2: Implement `/new [harness] [query...]` command in `main.go` to spawn a new tmux session and run the harness with the interactive prompt query.
